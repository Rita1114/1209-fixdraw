...
public class C2toMgr : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (GameMgr.第一次對話==true)
        {
            flowchart.SetBooleanVariable("第二次說",true);
        }//透過管理員紀錄設定，避免變數消失
        ...
        if (GameMgr.第二次對話==true)
        {
            flowchart.SetBooleanVariable("第三次說",true);
        }//透過管理員紀錄設定，避免變數消失
        
        
        if (flowchart.GetBooleanVariable("第三次說")==true)
        {
            GameMgr.第三次對話 = true;
        }//對話完畢給管理員知道
